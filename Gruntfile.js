  ...
  grunt.initConfig({
    ...
    watch: {
      ...
      injectPo: {
        files: [
          '<%= yeoman.client %>/**/*.po'],
        tasks: ['nggettext_compile', 'injector:scripts']
      },
      sass: {
        files: ['<%= yeoman.client %>/{app,components}/**/*.{scss,sass}'],
        tasks: ['sass', 'postcss']
    ...
    //Serve font end files in development mode
    connect: {
      options: {
        port: 9000,
        hostname: 'localhost',
    ...
    nggettext_compile: {
      all: {
        files: {
          '<%= yeoman.client %>/components/translations/translations.js': [
            '<%= yeoman.client %>/po/*.po'
          ]
        }
      },
