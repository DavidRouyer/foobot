    ...
    return {
      ...
      link: function(scope, element) {
        ...
        scope.render = function(alldatas) {
          ...
          svg.append('path')
              .datum(data)
              .attr('class', 'voc-area area')
              .attr('fill', 'url(' + location.href + '#voc-grad)')
              .attr('d', area);
          ...
          d3.select('.voc-area').on('mousemove', function(d, i) {
            ...
            d3.select(this)
              ...
              tooltip.html('<div class="tooltip-arrow"></div><div class="tooltip-inner"><p>' + yValue + ' ppb<br>' + moment(xValue).format('LLLL') + '</p></div>').style('visibility', 'visible');
          });
